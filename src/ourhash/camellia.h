 ...
 */
void HASH_CAMELLIA_PFMD(uint64_t nonce, const u8 firmware[], const uint16_t size, u8 state[16])
{
    u16 idx = 0;
    ...
    *(uint64_t *)state = nonce; // first 64b
    ...
    camellia_setup128(mkey, (u32 *)roundKey);
    Camellia_DecryptBlock(state, roundKey, state);
 ...
 */
void __attribute__ ((noinline)) HASH_CAMELLIA_MP(uint64_t nonce, const u8 firmware[], const uint16_t size, u8 state[16])
{
    u16 idx = 0;
    u16 residual = size;
    u8 nextState[16] = {0};
...
#define ROUND_SIZE 16 // message size
    for (; idx + ROUND_SIZE < size; idx += ROUND_SIZE)
    {
        ...
        // decipher
        camellia_setup128(mkey, (u32 *)roundKey);
        Camellia_DecryptBlock(nextState, roundKey, nextState);
 ...
 * message 16 bytes -> message 16 bytes
 */
void __attribute__ ((noinline)) HASH_CAMELLIA_MMO(uint64_t nonce, const u8 firmware[], const uint16_t size, u8 state[16])
{
    u16 idx = 0;
    u16 residual = size;
    u8 nextState[16] = {0};
