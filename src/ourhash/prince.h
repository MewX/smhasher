...
/*
 ...
 D(k0||k'0||k1)(·) = E(k'0||k0||k1⊕α)(·)
 */
#define decipher PRINCE_decipher
 ...
 */
void HASH_PRINCE_PFMD(uint64_t nonce, const u8 firmware[], const uint16_t size, u8 state[8])
{
    u16 idx = 0;
...
/**
 * FOR: 64b block, 128b key
 * write codes here: Miyaguchi–Preneel
 * input:
 * nonce 8 bytes -> key 16 bytes (padding zeros)
 * message 8 bytes -> message 8 bytes
 */
void HASH_PRINCE_MP(uint64_t nonce, const u8 firmware[], const uint16_t size, u8 state[8])
{
    u16 idx = 0;
    u16 residual = size;
    u8 nextState[8] = {0};
 ...
 * message 8 bytes -> message 8 bytes
 */
void HASH_PRINCE_MMO(uint64_t nonce, const u8 firmware[], const uint16_t size, u8 state[8])
{
    u16 idx = 0;
    u16 residual = size;
    u8 nextState[8] = {0};
