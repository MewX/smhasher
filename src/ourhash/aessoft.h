...
#define InvSBox AESSOFT_InvSBox
 ...
 */
void HASH_AES_PFMD(uint64_t nonce, const u8 firmware[], const uint16_t size, u8 state[16])
{
    u16 idx = 0;
 ...
 */
void __attribute__ ((noinline)) HASH_AES_MP(uint64_t nonce, const u8 firmware[], const uint16_t size, u8 state[16])
{
    u16 idx = 0;
    u16 residual = size;
    u8 nextState[16] = {0};
...
#define ROUND_SIZE 16 // message size
    for (; idx + ROUND_SIZE < size; idx += ROUND_SIZE)
    {
        ...
        // decipher
        AES_init(&ctx, mkey);
        AES_decrypt(&ctx, nextState, nextState);
 ...
 * message 16 bytes -> message 16 bytes
 */
void __attribute__ ((noinline)) HASH_AES_MMO(uint64_t nonce, const u8 firmware[], const uint16_t size, u8 state[16])
{
    u16 idx = 0;
    u16 residual = size;
    u8 nextState[16] = {0};
